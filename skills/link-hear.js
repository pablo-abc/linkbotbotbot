module.exports = controller => {
    controller.hears(/(<https?:\/\/((www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*))(\|\2)?>)/g, 
  ...
  controller.hears(/links channel( <#(.*)\|.*>)?/i, 
    ...
    .then(links => {
      if (links.length === 0) 
        return bot.reply(message, `No links found in <#${channel}>`);
  ...
  controller.hears(/links user( <@(.*)>)?/i,
    ...
    .then(links => {
      if (links.length === 0)
        return bot.reply(message, `No links found by <@${user}>`);
      const parsedLinks = links.reduce((result, link) => {
  ...
  controller.hears(/links count( (user <@(.*)>|(channel)( <#(.*)\|.*>)?))?/,
                   'direct_message,direct_mention,mention',
                   (bot, message) => {
    let options = {};
    let answer = ``;
