module.exports = controller => {
  const parseLinks = (bot, message, limit, links, answer) => {
    const parsedLinks = links.reduce((result, link) => {
      if (limit && link.createdAt < limit) return result
  ...
  controller.hears(
    ...
    (bot, message) => {
      ...
      if (message.match[10]) {
        options.channel = message.match[10]
  ...
  controller.hears(
    ...
    (bot, message) => {
      ...
      let answer = ``
      if (message.match[3]) {
        options = {user: message.match[3]}
        answer = ` by <@${message.match[3]}>`
      }
      if (message.match[6]) {
        options = {channel: message.match[6]}
